...
import { Calendar, Search, MapPin, Clock, Star, Dumbbell, Receipt, HelpCircle, CheckCircle, SlidersHorizontal, Map, Compass, Navigation, ShoppingCart, Layers, ChevronLeft } from 'lucide-react';
...
export const BookingTab: React.FC<BookingTabProps> = ({ sportFilter, setSportFilter }) => {
  const { courts, selectedDate, setSelectedDate, createBooking, bookings, cancelBooking, user, depositWallet, selectedCourtId, setSelectedCourtId } = useSport();
  ...
  const getBookingStatusMeta = (status: Booking['status']) => {
    switch (status) {
      case 'pending':
        return { label: 'Chờ duyệt', className: 'bg-amber-50 text-amber-700 border border-amber-100', active: false };
      case 'confirmed':
      ...
      case 'rejected':
        return { label: 'Bị từ chối', className: 'bg-red-50 text-red-600 border border-red-100', active: false };
      default:
  ...
  const [selectedSlots, setSelectedSlots] = useState<{ slot: TimeSlot; subCourtId: string }[]>([]);
  ...
  const datesPool = [0, 1, 2, 3, 4].map(idx => ({
  ...
  const handleSelectSlot = (slot: TimeSlot, subCourtId: string) => {
  ...
  const finalPrice = selectedSlot ? (selectedSlot.price + extrasPrice) : 0;

  const handleCheckoutSubmit = () => {
    if (selectedSlots.length > 0) {
      if (!expandedCourtId) return;
      const totalRequired = selectedSlots.reduce((sum, s) => sum + s.slot.price, 0) + extrasPrice;
      if (user.walletBalance < totalRequired) {
        setBookingError(`Số dư tài khoản ví không đủ. Cần thêm ${(totalRequired - user.walletBalance).toLocaleString('vi-VN')}đ.`);
        return;
      }
      ...
      for (let i = 0; i < selectedSlots.length; i++) {
        ...
        const extrasToApply = i === 0 ? { rackets: numRackets, water: numWater } : { rackets: 0, water: 0 };
        ...
        if (res.success && res.booking) {
          ...
          setBookingError(res.error || 'Đã xảy ra lỗi.');
          errorOccurred = true;
          break;
        }
      }

      if (!errorOccurred && lastBooking) {
        // Create an aggregate successful ticket for display
        const allTimes = selectedSlots.map(s => s.slot.time).join(', ');
        const allSubCourtsNames = selectedSlots.map(s => {
          const c = courts.find(court => court.id === expandedCourtId);
          const sub = c?.subCourts.find(sc => sc.id === s.subCourtId);
          return sub?.name || '';
        }).filter((v, idx, arr) => arr.indexOf(v) === idx).join(', ');

        const aggregateTicket: Booking = {
          ...lastBooking,
          timeSlot: allTimes,
          subCourtName: allSubCourtsNames,
          price: totalRequired
        };

        setActiveCreatedTicket(aggregateTicket);
        setBookingError(null);
      }
    } else {
      if (!selectedSlot || !expandedCourtId || !selectedSubCourtId) return;
      ...
      const res = createBooking(
        ...
        selectedDate,
        selectedSlot.id,
        { rackets: numRackets, water: numWater }
      );

      if (res.success && res.booking) {
        setActiveCreatedTicket(res.booking);
        setBookingError(null);
      } else {
        setBookingError(res.error || 'Đã xảy ra lỗi.');
      }
  ...
  const renderScheduler = () => {
    ...
    const selectedSubCourtsNames = selectedSlots.map(s => {
      const match = subCourtsList.find(sub => sub.id === s.subCourtId);
                  ...
                  <thead>
                    <tr className="bg-neutral-100/80">
                      ...
                      {standardSlots.map((slot) => {
                        ...
                        return (
                          <th key={slot.id} className="border-b border-neutral-150 p-2 text-[8px] font-black text-neutral-500 text-center min-w-[68px] tracking-tight">
                            {startTime}
                          </th>
                        );
                      })}
                      ...
                      return (
                        <tr key={subCourt.id}>
                          ...
                          {slotsForSub.map((slot, index) => {
                            const isSelected = selectedSlots.some(s => s.slot.id === slot.id && s.subCourtId === subCourt.id);
                            const isBooked = slot.isBooked;
                            const isLocked = !isSelected && !isBooked && (index === 0 || (subCourt.id.charCodeAt(subCourt.id.length-1) + index) % 5 === 0);
                            ...
                            return (
                              <td key={slot.id} className="p-0.5 border-b border-r border-neutral-150/40">
                                <button
                                  disabled={isBooked || isLocked}
                                  onClick={() => {
                                    if (isSelected) {
                                      setSelectedSlots(prev => prev.filter(s => !(s.slot.id === slot.id && s.subCourtId === subCourt.id)));
                                    } else {
                                      setSelectedSlots(prev => [...prev, { slot, subCourtId: subCourt.id }]);
            ...
            <div className="space-y-2.5 text-[11px] pt-1 font-semibold">
              ...
              <div className="flex justify-between items-center border-t border-dashed border-neutral-100 pt-2">
                ...
                <span className="font-mono font-extrabold text-emerald-600 text-[11px] text-right max-w-[200px] truncate">
                  {selectedSlots.length > 0 ? selectedSlots.map(s => s.slot.time).join(', ') : (selectedSlot ? selectedSlot.time : (
              ...
              <div className="pt-2.5 border-t border-solid border-neutral-150/70 flex justify-between items-center">
                ...
                <span className="text-sm font-black text-[rgb(16,185,129)]">
                  {selectedSlots.length > 0 
                    ? `${selectedSlots.reduce((sum, s) => sum + s.slot.price, 0).toLocaleString('vi-VN')}đ` 
                    : (selectedSlot ? `${selectedSlot.price.toLocaleString('vi-VN')}đ` : '0đ')}
                </span>
              </div>
            </div>
          </div>
  ...
  return (
    <div id="booking-tab-content" className="flex-1 flex flex-col overflow-hidden select-none animate-fadeIn bg-neutral-50/50 relative">
            ...
            >
              <div>
                ...
                <p className="text-[11px] font-extrabold text-emerald-400">
                  {selectedSlot.price.toLocaleString('vi-VN')} VND
                </p>
              </div>
                  ...
                  {b.status === 'confirmed' && (
                    <div className="pt-2.5 border-t border-neutral-100 flex flex-col items-center space-y-2">
                      ...
                      <div className="flex items-center justify-between w-full text-[9px]">
                        <span className="text-neutral-400">Giá: {b.price.toLocaleString('vi-VN')} VNĐ</span>
                        <button
                          onClick={() => {
                            if (confirm('Bạn có đồng ý hủy lịch đặt sân này không? Bạn sẽ được hoàn 85% chi phí về Ví SportRes trực tiếp.')) {
                              cancelBooking(b.id);
                            }
                          }}
      ...
      {showCheckoutModal && (selectedSlot || selectedSlots.length > 0) && (() => {
        const totalSlotsPrice = selectedSlots.length > 0 
          ? selectedSlots.reduce((sum, s) => sum + s.slot.price, 0)
        ...
        const displayTime = selectedSlots.length > 0 
          ? selectedSlots.map(s => s.slot.time).join(', ')
            ...
            <div className="bg-white rounded-3xl w-full max-w-md p-5 shadow-2xl border border-neutral-100 space-y-4 max-h-[90vh] overflow-y-auto">
              ...
              {!activeCreatedTicket ? (
                <>
                  ...
                  {/* Wallet evaluation */}
                  <div className="bg-neutral-50 p-2.5 rounded-xl flex items-center justify-between text-[10px]">
                    <span className="text-neutral-500">Ví khả dụng của bạn:</span>
                    <span className="font-extrabold text-neutral-800">{user.walletBalance.toLocaleString('vi-VN')}đ</span>
                  </div>

                  {user.walletBalance < checkoutFinalPrice && (
                    <div className="bg-red-50 text-red-600 p-2.5 rounded-xl text-[9px] space-y-1">
                      <p className="font-bold">⚠️ Số dư tài khoản không đủ để thanh toán!</p>
                      <button
                        onClick={() => depositWallet(200000)}
                        className="bg-red-600 text-white font-extrabold px-3 py-1 rounded text-[8px] hover:bg-red-500 transition cursor-pointer"
                      >
                        Nạp nhanh +200,000 VND
                      </button>
                    </div>
                  )}
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={user.walletBalance < checkoutFinalPrice}
                      className={`flex-1 py-2 rounded-xl text-xs font-bold text-white transition ${
                        user.walletBalance >= checkoutFinalPrice ? 'bg-emerald-600 hover:bg-emerald-500 cursor-pointer shadow-md' : 'bg-neutral-300 cursor-not-allowed'
                      }`}
                    >
                      Xác nhận thanh toán
                    </button>
                ...
                /* Success animated Ticket overlay! */
                <div className="space-y-4 text-center py-2 animate-scaleUp">
                  ...
                  <h4 className="text-xs font-black text-emerald-600 uppercase tracking-wider">Đặt sân thành công!</h4>
                  
                  {/* Physical Ticket Pass */}
                  <div className="bg-neutral-900 text-white p-4 rounded-2xl text-left border-2 border-emerald-500 relative overflow-hidden font-sans space-y-3 shadow-xl">
                    {/* Notch circles side cut */}
                    <div className="absolute top-1/2 -translate-y-1/2 -left-3 w-6 h-6 bg-white rounded-full"></div>
                    <div className="absolute top-1/2 -translate-y-1/2 -right-3 w-6 h-6 bg-white rounded-full"></div>

                    <div className="flex justify-between items-start">
                      <div>
                        <span className="text-[8px] tracking-widest text-emerald-400 font-mono">SPORTRES GATE PASS</span>
                        <h5 className="text-xs font-black text-neutral-100 leading-tight truncate max-w-[200px]">{activeCreatedTicket.courtName}</h5>
                      </div>
                      <span className="text-lg">🏟️</span>
                    </div>

                    <div className="grid grid-cols-2 gap-2 text-[9px] border-t border-b border-neutral-800 py-2.5">
                      <div>
                        <span className="text-neutral-400 block font-mono">PHÂN SÂN:</span>
                        <span className="font-extrabold text-neutral-100">{activeCreatedTicket.subCourtName}</span>
                      </div>
                      <div>
                        <span className="text-neutral-400 block font-mono">NGÀY ĐẶT:</span>
                        <span className="font-extrabold text-neutral-100">{activeCreatedTicket.date}</span>
                      </div>
                      <div className="mt-1">
                        <span className="text-neutral-400 block font-mono">KHUNG GIỜ:</span>
                        <span className="font-extrabold text-emerald-400">{activeCreatedTicket.timeSlot}</span>
                      </div>
                      <div className="mt-1">
                        <span className="text-neutral-400 block font-mono">CHECK-IN CODE:</span>
                        <span className="font-extrabold text-neutral-100">{activeCreatedTicket.qrcode}</span>
                      </div>
                    </div>

                    {/* Dynamic simulated scanning barcode */}
                    <div className="flex flex-col items-center justify-center space-y-1 pt-1">
                      <div className="h-6 w-full bg-repeating-linear-stripes text-black rounded font-mono text-[8px] flex items-end justify-center select-all">
                        || |||| | | |||| |||| | ||||||
                      </div>
                      <p className="text-[8px] text-neutral-400 text-center uppercase tracking-wide">Quét mã tại quầy để tiếp đón</p>
                    </div>
                  </div>

                  <p className="text-[9px] text-neutral-400">
                    Một bản vé điện tử Gate Pass kèm sơ đồ chỉ đường đã được gửi về email của bạn: **minhquan8805@gmail.com**.
                  </p>
