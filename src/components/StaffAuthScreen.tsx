...
import { Mail, Lock, Eye, EyeOff, ArrowLeft, ShieldCheck, UserCog, Info } from 'lucide-react';
...
export const StaffAuthScreen: React.FC<StaffAuthScreenProps> = ({ onLogin, onBack }) => {
  const { loginUser, demoUsers } = useSport();
    ...
    <div className="flex flex-col h-full bg-[#2f3136] text-white p-6 justify-center animate-fadeIn relative">
      ...
      <div className="w-full max-w-xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">Đăng nhập bằng tài khoản chủ sân, nhân viên hoặc admin đã được cấp.</p>
        </div>
        ...
        <div className="bg-[#36393f] rounded-2xl p-6 shadow-2xl">
          ...
          <p className="mt-4 text-xs text-gray-500 text-center">
            Nếu quên thông tin đăng nhập, vui lòng liên hệ admin để được cấp lại.
          </p>
          ...
          <div className="mt-5">
            ...
            {showDemoPanel && (
              <div className="mt-3 bg-[#202225] rounded-xl p-3 space-y-4">
                {[
                  ['Chủ sân', demoUsers.filter(item => item.role === 'venue_owner')],
                  ['Nhân viên', demoUsers.filter(item => item.role === 'staff')],
                  ['Quản trị viên', demoUsers.filter(item => item.role === 'admin')],
                ].map(([label, users]) => (
                  <div key={label}>
                    <p className="text-xs uppercase tracking-wide text-gray-500 font-bold mb-2">{label}</p>
                    <div className="space-y-2">
                      {(users as typeof demoUsers).map(userItem => (
                        <button
